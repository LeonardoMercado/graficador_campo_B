...
% Rango 1: 500 µA; Resolución: 0.01 µA; Exactitud: 0.6% + 20.
...
for i=1:1:length(I_rango_1)
    sensibilidad_rango_1(i) = sensibilidad(I_rango_1(i),0.01*1e-6,0.6,20);
end
% Rango 2: 5000 µA; Resolución: 0.1 µA.; Exactitud: 0.6% + 5.
I_rango_2 = [500*1e-6 1000*1e-6 2000*1e-6 3000*1e-6 4000*1e-6 5000*1e-6];
for i=1:1:length(I_rango_2)
    sensibilidad_rango_2(i) = sensibilidad(I_rango_2(i),0.1*1e-6,0.6,5);
end
% Rango 3: 50 mA; Resolución: 0.001 mA.; Exactitud: 0.6% + 20.
I_rango_3 = [5*1e-3 10*1e-3 20*1e-3 30*1e-3 40*1e-3 50*1e-3];
for i=1:1:length(I_rango_3)
    sensibilidad_rango_3(i) = sensibilidad(I_rango_3(i),0.001*1e-3,0.6,20);
end
% Rango 4: 400 mA; Resolución: 0.01 mA.; Exactitud: 0.6% + 5.
I_rango_4 = [50*1e-3 100*1e-3 150*1e-3 200*1e-3 250*1e-3 300*1e-3 350*1e-3 400*1e-3];
for i=1:1:length(I_rango_4)
    sensibilidad_rango_4(i) = sensibilidad(I_rango_4(i),0.01*1e-3,0.6,5);
end
% Rango 5: 5 A; Resolución: 0.0001 A.; Exactitud: 0.8% + 20.
I_rango_5 = [400*1e-3 600*1e-3 800*1e-3 1 2 3 5];
for i=1:1:length(I_rango_5)
    sensibilidad_rango_5(i) = sensibilidad(I_rango_5(i),0.0001,0.8,20);
end
% Rango 6: 10 A; Resolución: 0.001 A.; Exactitud: 0.8% + 5.
I_rango_6 = [5 6 7 8 9 10];
%-----
for i=1:1:length(I_rango_6)
    sensibilidad_rango_6(i) = sensibilidad(I_rango_6(i),0.001,0.8,5);
end
%-----
...
title("Sensibilidad para Fluke 289",'FontSize',14);
